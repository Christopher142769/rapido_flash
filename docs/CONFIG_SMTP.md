...
Si `SMTP_HOST` et `SMTP_USER` sont renseignés, les codes de connexion, les réponses aux formulaires personnalisés et les **notifications de commandes** (Shop + app) sont envoyés par e-mail. Destinataires des commandes : `PLATFORM_ADMIN_EMAIL` (voir [CONFIG_MAINTENANCE.md](./CONFIG_MAINTENANCE.md)). Les **alertes WhatsApp** utilisent `RAPIDO_WHATSAPP` (+229 40 31 75 68 par défaut) — voir [CONFIG_WHATSAPP_COMMANDES.md](./CONFIG_WHATSAPP_COMMANDES.md). Sinon, le code s’affiche dans la console du backend (mode dev).
