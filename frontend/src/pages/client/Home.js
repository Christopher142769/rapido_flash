        ...
        {!error && filteredStructures.length > 0 && (
          <div className="plats-grid-mobile structures-grid-mobile">
            {filteredStructures.map((structure) => {
              ...
              const bannerUrl = structure.banniere && !structure.banniere.includes('placeholder.com')
                ? `${BASE_URL}${structure.banniere}`
                : generateBannerPlaceholderSVG(0);
              return (
                <div
                  key={structure._id}
                  className="plat-card-mobile structure-card-mobile"
                  onClick={() => navigate(`/restaurant/${structure._id}`)}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/restaurant/${structure._id}`); }}
                >
                  <div className="structure-card-banner-wrap-mobile">
                    <img src={bannerUrl} alt={structure.nom} className="structure-banner-img-mobile" onError={(e) => { e.target.src = generateBannerPlaceholderSVG(0); }} />
                    <div className="structure-banner-overlay-mobile" aria-hidden />
                    <div className="structure-banner-text-mobile">
                      <span className="structure-banner-name-mobile">{structure.nom}</span>
                      {(structure.nom && (structure.description || structure.categorie?.nom)) ? ' et ' : ''}
                      {(structure.description || structure.categorie?.nom) || ''}
                    </div>
                    {structure.telephone && (
                      <div className="structure-contact-icons-mobile" onClick={(e) => e.stopPropagation()}>
                        <a href={`tel:${structure.telephone.trim()}`} className="structure-contact-icon-mobile" title={t('home', 'call')} aria-label={t('home', 'call')}>
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                        </a>
                        <a href={phoneToWa(structure.telephone) ? `https://wa.me/${phoneToWa(structure.telephone)}` : '#'} target="_blank" rel="noopener noreferrer" className="structure-contact-icon-mobile structure-contact-icon-wa-mobile" title={t('home', 'whatsapp')} aria-label={t('home', 'whatsapp')}>
                  ...
                  <div className="structure-card-body-mobile">
                    <div className="structure-card-logo-mobile">
                      {logoUrl ? (
                        <img src={logoUrl} alt={structure.nom} onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling?.classList.add('show'); }} />
                      ) : null}
                      <span className="structure-card-logo-placeholder-mobile" style={{ display: logoUrl ? 'none' : 'flex' }}>🏪</span>
                    </div>
                    <div className="structure-card-info-mobile">
                      ...
                      <p className="structure-card-meta-mobile">
                        {structure.position?.adresse || (structure.categorie && structure.categorie.nom) || '—'}
                      </p>
          ...
          {!error && filteredStructures.length > 0 && (
            <div id="section-structures" className="structures-grid-desktop">
              {filteredStructures.map((structure) => {
                ...
                const distanceKm = structure.distance != null ? structure.distance.toFixed(2) : null;
                const logoUrl = structure.logo ? `${BASE_URL}${structure.logo}` : null;
                const bannerUrl = structure.banniere && !structure.banniere.includes('placeholder.com') ? `${BASE_URL}${structure.banniere}` : generateBannerPlaceholderSVG(0);
                return (
                  <div
                    key={structure._id}
                    className="structure-card-desktop"
                    onClick={() => navigate(`/restaurant/${structure._id}`)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/restaurant/${structure._id}`); }}
                  >
                    <div className="structure-card-banner-wrap">
                      <img src={bannerUrl} alt={structure.nom} className="structure-card-banner" onError={(e) => { e.target.src = generateBannerPlaceholderSVG(0); }} />
                      <div className="structure-banner-overlay-desktop" aria-hidden />
                      <div className="structure-banner-text-desktop">
                        <span className="structure-banner-name-desktop">{structure.nom}</span>
                        {(structure.nom && (structure.description || structure.position?.adresse || structure.categorie?.nom)) ? ' et ' : ''}
                        {(structure.description || structure.position?.adresse || structure.categorie?.nom) || ''}
                      </div>
                      {structure.telephone && (
                        <div className="structure-contact-icons-desktop" onClick={(e) => e.stopPropagation()}>
                          <a href={`tel:${structure.telephone.trim()}`} className="structure-contact-icon-desktop" title={t('home', 'call')} aria-label={t('home', 'call')}>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                          </a>
                          <a href={phoneToWa(structure.telephone) ? `https://wa.me/${phoneToWa(structure.telephone)}` : '#'} target="_blank" rel="noopener noreferrer" className="structure-contact-icon-desktop structure-contact-icon-wa-desktop" title={t('home', 'whatsapp')} aria-label={t('home', 'whatsapp')}>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                          </a>
                        </div>
                      )}
                    ...
                    <div className="structure-card-body">
                      <div className="structure-card-logo-wrap">
                        <div className="structure-card-logo">
                          {logoUrl ? <img src={logoUrl} alt={structure.nom} onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling?.classList.add('show'); }} /> : null}
                          <span className="structure-card-logo-placeholder" style={{ display: logoUrl ? 'none' : 'flex' }}>🏪</span>
                        </div>
