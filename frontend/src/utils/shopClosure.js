/** Fermeture quotidienne — aligné sur backend/utils/shopClosure.js */
...
export function normalizeTimeHHmm(str) {
...
export function getShopClosureState(product, now = new Date()) {
  ...
  if (manualOverride === 'open') {
    return {
      ...
      closureClosedFrom: null,
      closureClosedUntil: null,
      closureReopensAt: scheduleClosed ? null : nextDailyTimeIso(now, dailyCloseTime),
      closureMessage: message,
      closureRemainingMs: 0,
      dailyCloseTime,
      dailyOpenTime,
  ...
  return {
    ...
    isClosurePending: !scheduleClosed,
    isClosureScheduled: true,
    closureEnabled: true,
    closureClosedFrom: null,
    closureClosedUntil: reopenAt,
...
export function closurePayloadFromForm(shopClosure) {
  ...
  return {
    enabled: !!c.enabled,
    dailyCloseTime: normalizeTimeHHmm(c.dailyCloseTime) || '',
    dailyOpenTime: normalizeTimeHHmm(c.dailyOpenTime) || '',
    message: String(c.message || '').trim().slice(0, 500),
    manualOverride: null,
  };
}

export function isoToDailyTime(iso) {
  if (!iso) return '';
  const parts = new Intl.DateTimeFormat('en-GB', {
    timeZone: SHOP_TZ,
    hour: '2-digit',
...
export function formatDailyTime(timeHHmm) {
