  ...
  return (
    <nav className="bottom-navbar-mobile">
      ...
      >
        ...
        <span className="nav-label">{t('nav', 'cart')}</span>
      ...
      <button
        type="button"
        className={`nav-item ${location.pathname === '/factures' || location.pathname.startsWith('/facture/') ? 'active' : ''}`}
      ...
      {/* Bouton d'installation - uniquement sur la page Home */}
      {location.pathname === '/home' && <InstallButton variant="navbar" />}
    </nav>
  );
};
